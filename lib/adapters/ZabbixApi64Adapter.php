...
class ZabbixApi64Adapter extends ZabbixApiAdapter
{
    ...
    public function getUserType(string $username): int
    {
        ...
        // Tentar com selectRole primeiro, fallback para type direto
        ...
        if (!empty($user_info[0])) {
            ...
            // Normalizar para escala clássica: 1=User, 2=Admin, 3=Super Admin
            if (isset($u['role']['type'])) {
                $rt = (int)$u['role']['type'];
                // Se já vier na escala 1-3 (alguns builds), não somar
                return $rt >= 1 ? $rt : $rt + 1;
            }
