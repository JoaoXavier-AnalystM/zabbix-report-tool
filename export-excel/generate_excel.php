...
try {
    ...
    switch ($reportType) {
        ...
        case 'inventory':
            ...
            // Chamada principal com busca por substring (funciona para a maioria)
            ...
            // Método: calcula downtime real a partir de triggers ICMP e subtrai do período total
            ...
            if(isset($columns['availability'])){
                ...
                if(!empty($avail_trigger_map)){
                    ...
                    if($is_v60_excel){
                        ...
                        // Também buscar eventos ativos agora (sem time_till)
            ...
            // trend.get só retorna dados com granularidade de 1 hora e precisa
                ...
                if($use_history){
                    ...
                    foreach([0, 3] as $hist_type){
                        ...
                        foreach($grouped as $iid=>$vals){
                            ...
                            // Merge: se já existia por outro tipo, pegamos o mais extremo
            ...
            foreach ($hostIds as $hid) {
                ...
                if(isset($columns['disks'])){
                    // Regex único que captura o filesystem e o sufixo para os três padrões:
                    //   vfs.fs.size[/mount,total]
                    //   vfs.fs.dependent.size[/mount,total]
                    //   vmware.vm.vfs.fs.size[url,vm,/mount,total]  ← último parâmetro é o sufixo
                    ...
                    foreach($host_items_map as $k=>$i){
                        ...
                        // Construir a key de "used" substituindo apenas o sufixo final
                        $used_k = preg_replace('/,total\]$/', ',used]', $k);
                        $used   = isset($host_items_map[$used_k]) ? (int)$host_items_map[$used_k]['lastvalue'] : 0;

                        // Mostrar apenas o mount point (último segmento do parâmetro)
        ...
        case 'peaks_report':
            ...
            // CPU Windows: perf_counter (apenas se não tiver cpu.util)
            ...
            // Se houver vários itens que dão match para o mesmo host (ex: cpu.util e cpu.util[,user])
            // escolher o melhor: primeiro % de unidades, depois key mais curta (menos params = mais genérica)
            ...
            // Windows perf_counter apenas para hosts que ainda não têm cpu.util
            ...
            // Memória "disponível" apenas para hosts sem item de "usada"
            ...
            // ── getDailyPeaks: SEMPRE history.get para precisão exata ────
            // trend.get NÃO é confiável para peaks porque value_max é o máximo
            ...
            $getDailyPeaks = function(array $item, int $from_ts, int $to_ts, bool $invert = false) use ($api): array {
                ...
                try {
                    ...
                    // Limit dinâmico: ~1 dado/min * 1440 min/dia * dias, máximo 50000
