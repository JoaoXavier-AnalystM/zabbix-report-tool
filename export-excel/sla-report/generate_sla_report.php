...
/* title/sub sempre brancos sobre o background */
...
<div class="wrap">
  ...
  <div class="table-wrap">
    ...
    foreach ($data as $row) {
        ...
        // ── Eventos na mesma linha (primeiro) ou linhas seguintes ──────────
        ...
        if ($hasEvents) {
            ...
            foreach ($events as $idx => $ev) {
                ...
                if ($idx === 0) {
                    // Primeira linha: repetir host com rowspan se houver vários eventos
                    $rc = count($events);
...
try {
    ...
    // Detectar versão do Zabbix para compatibilidade
    ...
    if (!empty($trigger_ids)) {
        if ($is_v60) {
            ...
            // problem.get pode não retornar todos os eventos dependendo do tipo de host
            ...
            // 3. event.get com intervalo — eventos resolvidos do período
            ...
            // 4. event.get sem time_till — eventos ativos agora (não têm r_eventid)
            ...
            // Construir mapa eventid->r_eventid a partir de problem.get (mais confiável no 6.0)
            $prob_recovery_map = [];
            foreach (array_merge(
                is_array($problems_raw)    ? $problems_raw    : [],
                is_array($active_problems) ? $active_problems : []
            ...
            foreach ($all_problems as $p) {
                ...
                // Incluir si:
                // - Começou antes do fim do período (caso normal), OU
                // - Está ativo agora (r_eid=0) e começou antes de now (cobre eventos muito recentes)
    ...
    foreach ($events as $event) {
        ...
        // Calcular a parte do evento que cai dentro do período
        $overlapStart = max($startTime, $from_ts);
        $overlapEnd = min($endTime, $to_ts);
            
        // Só processar se houver sobreposição com o período
        if ($overlapEnd > $overlapStart) {
            $duration = $overlapEnd - $overlapStart;
            $downtime_by_hostid[$hostId] = ($downtime_by_hostid[$hostId] ?? 0) + $duration;

            // Só guardar o evento se a sobreposição for significativa ou se o evento estiver ativo
            if ($duration > 0) {
    ...
    foreach ($host_map as $hid => $hostName) {
        ...
        if (empty($hostEvents)) {
            // Host sem problemas no período - 1 linha
            $final_report_data[] = [
