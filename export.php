...
<script>
  ...
  (() => {
    ...
    const populateTemplates = (filter = '') => {
        ...
        pageData.forEach(item => {
            const label = document.createElement('label');
            label.className = 'chk';
            // Criar checkbox via DOM para poder setar .checked como propriedade JS
            // (usar atributo HTML 'checked' como string pode falhar em alguns browsers)
            const cb = document.createElement('input');
            cb.type = 'checkbox';
        ...
        // Atualizar badge com seleção atual cada vez que renderizar
        updateTmplBadge();
    };

    // Mostra quantos templates estão selecionados no botão seguinte
  ...
  // Polling a cada 200ms - lê diretamente do DOM via defaultValue fallback
  var _prev = '';
