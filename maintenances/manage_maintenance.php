...
if (empty($_SESSION['zbx_auth_ok'])) {
    file_put_contents($debugFile, date('Y-m-d H:i:s') . " - ERRO: Sessão inválida\n", FILE_APPEND);
    http_response_code(403);
    die('Invalid session');
}

// Obter tipo de usuário SEMPRE da API
require_once __DIR__ . '/../config.php';
...
// Detectar versão do Zabbix se não estiver na sessão
if (!isset($_SESSION['zabbix_version'])) {
    try {
        ...
        error_log("Erro detectando versão do Zabbix: " . $e->getMessage());
    }
}
...
// Verificar permissões conforme a ação
...
if ($action_type === 'create') {
    ...
    file_put_contents($debugFile, date('Y-m-d H:i:s') . " - ERRO: Ação inválida\n", FILE_APPEND);
...
// ============================================================
// FUNÇÃO UNIVERSAL - Converte usando o fuso horário do servidor
// ============================================================
...
// 🔥 CORREÇÃO: Removida a inclusão automática de siblings
...
try {
    ...
    if ($action_type === 'create') {
        ...
        throw new RuntimeException("Ação inválida: $action_type.");
    }

    file_put_contents($debugFile, date('Y-m-d H:i:s') . " - Sucesso! Mostrando página de resultado\n", FILE_APPEND);
    ...
    file_put_contents($debugFile, date('Y-m-d H:i:s') . " - EXCEÇÃO: " . $e->getMessage() . "\n" . $e->getTraceAsString() . "\n", FILE_APPEND);
